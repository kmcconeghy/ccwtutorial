...
ii) Estimate PLR for treatment initiation A. compare KM / PLR estimates
iii) estimate final treatment model A. Join treatment probabilities to outcome data B. compute IPCW
iv) examine IPCW weights for extreme values, non-sensical values
v)  examine balance in covariates across time
vi) generate "table 1" with some weighted difference statistic (e.g. wSMD)
...
The data was designed so treatment has no causal effect on outcome, but confounding is present [Data](01_syndata.v2.qmd). When you adjust for the confounders the risks are similar in both groups.

```{r }
#| label: cox-mod
#| echo: FALSE
d_cox = coxph(Surv(t_clone, event_outc) ~ assign + female + age + dx_1 + dx_2, data = d_cloned) 
...
#| label: plr-naive-est
  ...
  d_plr_naive_est = d_panel_outc %>% # <4>
...
#| fig-cap: "Naive PLR versus K-M estimator"
#| echo: FALSE
d_ci_cmp = inner_join(
...
# defined above
  d_glm = glm(event_outc ~ poly(time, 2, raw=T)*assign + female + age + dx_1 + dx_2, 
              data=d_panel_outc, family=binomial()) # <1>

  d_panel_outc$p.event0 <- predict(d_glm, mutate(d_panel_outc, assign=0), type="response") # <2>
  d_panel_outc$p.event1 <- predict(d_glm, mutate(d_panel_outc, assign=1), type="response") # <2>
  
  d_panel_outc[, `:=`(pr_surv_1 = cumprod(1 - p.event1)), by=list(id, assign)] # <3>
  d_panel_outc[, `:=`(pr_surv_0 = cumprod(1 - p.event0)), by=list(id, assign)] # <3>
  
  d_plr_adj_est = d_panel_outc %>% # <4>
    group_by(time) %>% # <4>
...
3.  Estimatie cumulative survival using cumulative product, within person-clone
4.  Summarize mean survival rates by time-period

::: callout-note
Note the outcome regression without covariates is just for comparison and understanding, the final analysis must include probability weights for artificial censoring even if no confounding.
:::
...
#| message: FALSE
#| warning: FALSE
  d_cox = coxph(Surv(enter, exit, event_outc) ~ assign + female + age + dx_1 + dx_2, data = d_panel_outc) # <1>
...
1.  Cox model, with time1, time2 parameters
...
d_gg_ci = d_ci_cmp %>%
      ggplot(aes(x=time)) +
      geom_line(aes(y = plr_0), color='#A60A3D', linetype=2) +
      geom_line(aes(y = cox_0), color='red', linetype=1) + 
      geom_line(aes(y = plr_1), color='navyblue', linetype=2) +
      geom_line(aes(y = cox_1), color='blue', linetype=1) + 
      scale_x_continuous(breaks = seq(0, 60, 6),
                         limits = c(0, 60)) +
 ...
 d_gg_rr = d_ci_cmp %>%
      ggplot(aes(x=time)) +
      geom_line(aes(y = cox_cid), color='green', linetype=1) + 
      geom_line(aes(y = plr_cid), color='forestgreen', linetype=2) +
      scale_x_continuous(breaks = seq(0, 60, 6),
                         limits = c(0, 60)) +
...
#| warning: FALSE
  ...
  d_glm_wt = glm(event_treat ~ poly(time, 2, raw=T), data=d_panel_treat, family=binomial()) # <1>
  d_panel_outc$pr_censnum = predict(d_glm_wt, newdata = d_panel_outc, type='response') # <1>

  # Denominator 
  d_glm_wt = glm(event_treat ~ poly(time, 2, raw=T) + female + 
                   dx_1 + age + dx_2, data=d_panel_treat, family=binomial()) # <2>
  
  d_panel_outc$pr_censdenom = predict(d_glm_wt, newdata = d_panel_outc, type='response') # <2>

  #d_panel_treat$pr_treat = d_glm_wt$fitted.values # <2>
  #setDT(d_panel_treat)
  #d_panel_treat[, cumpr_notreat := cumprod(1-pr_treat), by = .(id)] # <3>
  #d_panel_outc = left_join(d_panel_outc, 
  #                         select(d_panel_treat, id, time, pr_treat, 
  #                                cumpr_notreat), by=c('id', 'time')) # <4>
```

1. Marginal probability of no censoring
2. Conditional probability (covariates) for no censoring
2.  Add fitted probabilities to outcome dataset
...
#| label: cpt-weights
  ...
  d_panel_outc[, ipw := fcase(
    assign==0, (1-pr_censnum) / (1-pr_censdenom), # <1>
    assign==0 & t_treat==time, 0,
    assign==1 & time < 12, 1, # <2>
    assign==1 & time == 12  & t_treat   < 12, 1, # <3> 
    assign==1 & time == 12  & t_treat   ==12, pr_censnum / (pr_censdenom), # <4> 
    assign==1 & time == 12  & t_treat    >12, 0, # <5> 
    assign==1 & time > 12, 1 # <6>
  )]
  d_panel_outc[, ipw := cumprod(ipw), by=list(id, assign)] # <7>
...
#| label: desc-ipw
#| echo: false
  summary(d_panel_outc$ipw[d_panel_outc$ipw!=0])
```
...
#| label: est-plr-wt
  ...
  d_panel_outc$pr_1 = predict(d_glm_pe_1, newdata = d_panel_outc, type='response') # <2>
  d_panel_outc$pr_0 = predict(d_glm_pe_0, newdata = d_panel_outc, type='response') # <2>
...
#| label: survprob
#| 
  d_panel_outc[, `:=`(pr_cum_1 = cumprod(pr_1)), by=list(id, assign)] # <1>
  d_panel_outc[, `:=`(pr_cum_0 = cumprod(pr_0)), by=list(id, assign)] # <1>
  
  d_plrwt_est = d_panel_outc %>% # <2>
    group_by(time) %>% # <2>
...
#| echo: false
  ...
  ### Plot ----
    ...
    d_gg_rr = d_plrwt_est %>%
      ggplot(aes(x=time)) +
      geom_line(aes(y = cir), color='#228b22', linewidth = 1, linetype=4) + 
      geom_hline(aes(yintercept = 1), color='black', linewidth = 0.5, linetype=3) + 
      scale_y_continuous(limits = c(0.5, 1.5)) + 
      scale_x_continuous(breaks = seq(0, 60, 6),
                         limits = c(0, 60)) +
      theme_bw() +
      labs(x = 'Follow-up', y = 'Relative Risk', caption = '12-month grace period')
...
kables(
  list(d_all_est %>%
         ...
         kable_styling() %>%
         add_header_above(., c("No treatment initiation ever" = 5)),
       d_all_est %>%
         pivot_wider(., id_cols = time, names_from = Model, values_from = pr_ev_1) %>%
         kable(digits=3, align = 'c') %>%
         kable_styling() %>%
         add_header_above(., c("Treatment initiation at 12 months" = 5)),
       d_all_est %>%
         pivot_wider(., id_cols = time, names_from = Model, values_from = cid) %>%
         kable(digits=3, align = 'c') %>%
         kable_styling() %>%
         add_header_above(., c("Risk Differences" = 5)),
       d_all_est %>%
         pivot_wider(., id_cols = time, names_from = Model, values_from = cir) %>%
         kable(digits=3, align = 'c') %>%
         kable_styling() %>%
         add_header_above(., c("Relative Risks" = 5))
)) 
```

The weighted analysis observes a similar effects to time-dependent cox model.
