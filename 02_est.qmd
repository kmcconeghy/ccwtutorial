...
<!-- -->
  ...
  ii) Estimate PLR for treatment initiation
    A. compare KM / PLR estimates
  iv) estimate final treatment model 
    A. Join treatment probabilities to outcome data
    B. compute IPCW
...
# defined above
  d_glm = glm(event_outc ~ poly(time, 2, raw=T)*assign + female + age + dx_1 + dx_2, 
              data=d_panel_outc, family=binomial()) # 

  d_panel_outc$p.event0 <- predict(d_glm, mutate(d_panel_outc, assign=0), type="response")
  d_panel_outc$p.event1 <- predict(d_glm, mutate(d_panel_outc, assign=1), type="response")
  
  d_panel_outc[, `:=`(pr_surv_1 = cumprod(1 - p.event1)), by=list(id, assign)] # 
  d_panel_outc[, `:=`(pr_surv_0 = cumprod(1 - p.event0)), by=list(id, assign)] # 
  
  d_plr_adj_est = d_panel_outc %>% # 
    group_by(time) %>% # 
...
#| label: est-censor-prob
  d_glm_wt = glm(event_treat ~ poly(time, 2, raw=T) + female + 
                   dx_1, data=d_panel_treat, family=binomial()) # <1>
  d_panel_treat$pr_treat = d_glm_wt$fitted.values # <2>
  setDT(d_panel_treat)
  d_panel_treat[, cumpr_notreat := cumprod(1-pr_treat), by = .(id)] # <3>
  d_panel_outc = left_join(d_panel_outc, select(d_panel_treat, id, time, pr_treat, cumpr_notreat), by=c('id', 'time')) # <4>
```
...
#| label: cpt-weights
  ...
  d_panel_outc[, ipw := fcase(
    ...
    assign==1 & time ==12   & t_treat   ==12, 1 / (pr_treat), # <4> 
...
#| 
 d_all_est = 
  ...
  dplyr::filter(time %in% c(6, 12, 24, 36, 48, 60))
...
kables(
  list(d_all_est %>%
         ...
         kable_styling() %>%
         add_header_above(., c("No treatment initiation ever" = 4)),
       d_all_est %>%
         pivot_wider(., id_cols = time, names_from = Model, values_from = pr_ev_1) %>%
         kable(digits=3, align = 'c') %>%
         kable_styling() %>%
         add_header_above(., c("Treatment initiation at 12 months" = 4)),
       d_all_est %>%
