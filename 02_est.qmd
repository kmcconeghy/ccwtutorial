...
d_cox = coxph(Surv(t_clone, event_outc) ~ assign + female + age + + CC1 + CC2 + CC3 + CC4 + CC5, data = d_cloned) 
...
#| message: FALSE
#| warning: FALSE
  d_cox = coxph(Surv(enter, exit, event_outc) ~ assign + female + age + CC1 + CC2 + CC3 + CC4 + CC5, data = d_panel_outc) # <1>
...
#| label: exp-dta-censor
  ...
  d_panel_treat = select(d_panel, id, time, event_treat, t_treat, enter, exit, end, age, female, CC1:CC5, X1, X2) # <1>
```
...
#| warning: FALSE
  ...
  d_glm_wt = glm(event_treat ~ poly(time, 2, raw=T) + female + age + 
                   CC1 + CC2 + CC3 + CC4 + CC5 + X1, data=d_panel_treat, family=binomial()) # <2>
...
#| label: cpt-weights
  ...
  d_panel_outc[, ipw := fcase(
    ...
    assign==1 & time == 12  & t_treat  >12 & event_outc==0, 0, # <5> 
    assign==1 & time == 12  & t_treat  >12 & event_outc==1, 1, # <6> 
...
#| label: desc-marg_ipw
#| echo: false
  summary(d_panel_outc$marg_ipw[d_panel_outc$marg_ipw!=0])
```
...
#| echo: false
  summary(d_panel_outc$ipw[d_panel_outc$ipw!=0 & d_panel_outc$assign==1])
```
...
#| echo: false
  ...
  d_glm_pe_1 = glm(event_outc==0 ~ poly(time, 2, raw=T), 
                     data=d_panel_outc[assign==1, ], 
                     family=binomial(), weights = marg_ipw) # <1>

  d_glm_pe_0 = glm(event_outc==0 ~ poly(time, 2, raw=T), 
                   data=d_panel_outc[assign==0, ], 
                     family=binomial(), weights = marg_ipw) # <1>
  ...
  d_plrstabwt_est = d_panel_outc %>% # <2>
    group_by(time) %>% # <2>
    summarize(pr_ev_1 = mean(1-pr_cum_1), 
              pr_ev_0 = mean(1-pr_cum_0),
              .groups = 'drop') %>% # <2>
