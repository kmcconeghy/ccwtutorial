...
#| label: est-censor-prob
  d_glm_wt = glm(event_treat ~ poly(time, 2, raw=T) + poly(age, 2) + female + 
                   dx_1 + dx_2, data=d_panel_treat, family=binomial()) # <1>
...
#| 
 d_all_est = 
  bind_rows(
    select(d_km_est, time, pr_ev_0, pr_ev_1, cid) %>%
        mutate(Model = 'Kaplan-Meier'),
    select(d_plr_naive_est, time, pr_ev_1, pr_ev_0, cid) %>%
      mutate(Model = 'PLR-Naive'),
    select(d_lprwt_est, time, pr_ev_1, pr_ev_0, cid) %>%
