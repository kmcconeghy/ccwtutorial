...
dta_ay[, ipw := fcase(
  ...
  dta_ay[, marg_ipw := fcase(
    ...
    assign==0 & censor==1, 0,  
    assign==1 & censor==1, 0,  
    assign==1 & time < 12, 1 / 1, 
    assign==1 & time == 12  & t_treat  < 12, 1 / 1,  
    assign==1 & time == 12  & t_treat  >=12 & censor==0, pr_cens_num / (pr_cens_den), # <2> 
    assign==1 & time > 12, 1 
  )]
  
  dta_ay[, `:=`(ipw = cumprod(ipw), 
...
It can be observed the median is 1, and the mean is 2, the data is heavily skewed. Because the weight is the inverse probability of remaining uncensored and the you only estimate it for the uncensored risk set (where outcome risk is correlated with treatment), it is not expected to be mean 1. However the sum of the weights / (# observations) ~ 1.  

You can plot the counts across time for:

```{r }
#| label: desc-ipw-time
#| fig-cap: Weighted versus Unweighted Sample Across Time
#| fig-cap-location: top
#| warning: false
n_base = nrow(dta_ay[assign==1 & time==1, ])
dta_ay %>%
  group_by(assign, time) %>%
  summarize(n = n(),
            events = sum(event),
            sum_wts = sum(ipw),
            base = n_base, .groups = 'drop') %>%
  group_by(assign) %>%
  mutate(events = cumsum(events),
         base = n_base - lead(events),
         mean = sum_wts / base) %>%
 ...
 ggplot(., aes(x = time, group = assign, color = assign)) +
  ...
  labs(x = "Follow-up", y = "Weighted Sample / Umweighted - Events") +
  theme_bw()
```
...
```{r }
#| label: desc-marg_ipw-time
#| fig-cap: Average Weights Across Time
#| fig-cap-location: top
#| warning: false
dta_ay %>%
  dplyr::filter(ipw!=0) %>%
  group_by(assign, time) %>%
  summarize(mean = mean(marg_ipw), .groups = 'drop') %>%
  mutate(assign = factor(assign)) %>%
 ggplot(., aes(x = time, group = assign, color = assign)) +
  geom_line(aes(y = mean, color = assign)) +
  scale_color_manual( values = cbbPalette) +
  geom_hline(aes(yintercept=1),linewidth=1.1, linetype=3) +
  scale_x_discrete(labels = seq(0, 60, 12)) +
  labs(x = "Follow-up", y = "Mean (Stabilized Weights)") +
  theme_bw()
```