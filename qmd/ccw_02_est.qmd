...
# assign == 0
  ...
  d_glm_cwt_den = glm(treat ~ poly(time, 2, raw=T) + X + X_t + age + poly(age, 2) + female, # <1>
...
#| label: cpt-weights-st
  ...
  dta_c_panel[, marg_ipw := fcase(
    assign==0 & censor==0, (1-pr_cens_num) / (1-pr_cens_den), # <2>
    assign==0 & censor==1, 0,  
    assign==1 & censor==1, 0,  
    assign==1 & time < 12, 1, 
    assign==1 & time == 12  & t_treat  < 12, 1,  
    assign==1 & time == 12  & t_treat  >=12 & censor==0, pr_cens_num / (pr_cens_den), # <2> 
    assign==1 & time > 12, 1 
  )]
```
