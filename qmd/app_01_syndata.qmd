...
d_panel[, X_shift := cumsum(rbinom(.N, 1, 0.005)), by= list(id)]
d_panel[, X_t := fifelse((X + X_shift)>=1, 1L, 0L), by= list(id)]
```
...
d_panel_2 = d_panel %>% 
    # Treat event # <1>
    mutate(log_odds = -4.1 + 0.0001*time + -0.00001*(time^2) + 0.9*X + 0.9*X_t,
...
# Dataset with no A -> Y effect
  d_xy = d_panel_2 %>%
    mutate(model = 'xy') %>%
    # Outcome event # <2>
    mutate(log_odds = -5 + 0.007*time + 0.0002*(time^2) + 0.4*X + 0.7*X_t + 
...
# Dataset with A -> Y effect
  d_ay = d_panel_2 %>%
    ...
    # Outcome event # 
    mutate(log_odds = -5 + 0.007*time + 0.0002*(time^2) + 0.4*X + 0.7*X_t + 
                                  -0.2*female + 0.07*age + -0.001*(age^2) + treat*-0.8, # <4>
           p = exp(log_odds) / (1 + exp(log_odds)),
           outcome = rbinom(n(), size = 1, prob = p),
           outcome = if_else(outcome==1, 1, NA_integer_)
    ) %>%
...
data_cloned = bind_rows( # <1>
  ...
  ungroup %>%
  select(model, id, time, t_outcome, t_treat, t_censor, censor, event, assign, enter, exit, everything(), -fup, -X_shift, -log_odds, -p)
```
...
::: callout-note
Note how we do not specific death or some other censoring event, these complicate things and are common in applied work. I keep it simple here. 
:::
