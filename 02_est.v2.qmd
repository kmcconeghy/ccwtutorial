...
The data was designed so treatment has no causal effect on outcome, but confounding is present [Data](01_syndata.v2.qmd). Adjustment for confounders identifies similar risks by treatment assignment.
...
d_cox = coxph(Surv(time, event) ~ assign + female + age + X, data = dta_c_person) 
...
#| label: plr-naive-est
  ...
  # defined above
  d_glm = glm(event==0 ~ poly(time, 2, raw=T)*assign, data=dta_c_panel[censor==0, ], family=binomial()) # <1>
...
4.  Summarize by group/time as described above for the KM estimator.
...
# defined above
  d_glm = glm(event==0 ~ poly(time, 2, raw=T)*assign + female + age + X, 
              data=dta_c_panel[censor==0, ], family=binomial()) # <1>
...
#| message: FALSE
#| warning: FALSE
  d_cox = coxph(Surv(enter, exit, event_outc) ~ assign + female + age + X, data = dta_c_panel[censor==0, ]) # <1>

  d_cox_est = surv_adjustedcurves(fit = d_cox, variable = "assign", data = dta_c_panel[censor==0, ]) %>% # <2>
...
2.  We are estimating time to treatment, not outcome. The clone, assign=0 is already set up for this, so this line of code should be unnecessary but I leave in to illustrate point. 
...
#| warning: FALSE
  ...
  d_glm_wt = glm(treat ~ splines::bs(time, df=6), 
                 data=d_treat, family=binomial()) # <1>
  
  dta_c_panel$pr_censnum = predict(d_glm_wt, newdata = dta_c_panel, type='response') # <1>

  # Denominator 
  d_glm_wt = glm(treat ~ splines::bs(time, df=6) + X + X_t, 
                 data=d_treat, family=binomial()) # <2>
...
#| label: cpt-weights
  ...
  dta_c_panel[, marg_ipw := fcase(
    ...
    assign==1 & time == 12 & t_treat   ==12 & censor==0, pr_censnum / (pr_censdenom), # <9>
    default = ipw
  )]
  ...
  dta_outcome = dta_c_panel[censor==0, ]
```
...
8.  After setting these conditions, we compute the cumulative product of the weights.
9.  Marginal IPW is same, except numerator probability replaces 1
...
#| label: desc-ipw
#| echo: false
  summary(dta_outcome$ipw[dta_outcome$ipw!=0])
```

```{r }
#| label: desc-marg_ipw
#| echo: false
  summary(dta_outcome$marg_ipw[dta_outcome$marg_ipw!=0])
```
...
#| echo: false
  summary(dta_outcome$ipw[dta_outcome$ipw!=0 & dta_outcome$assign==1 & dta_outcome$time==12])
```
...
#| warning: FALSE  
#| 
  d_glm_pe_1 = glm(event==0 ~ poly(time, 2, raw=T) + female + age + X, 
                   data=dta_c_panel[assign==1 & censor==0, ], 
                   family=binomial(), weights = ipw) # <1>

  d_glm_pe_0 = glm(event==0 ~ poly(time, 2, raw=T) + female + age + X, 
                   data=dta_c_panel[assign==0 & censor==0, ], 
                     family=binomial(), weights = ipw) # <1>
  
  dta_c_panel$pr_1 = predict(d_glm_pe_1, newdata = dta_c_panel, type='response') # <2>
  dta_c_panel$pr_0 = predict(d_glm_pe_0, newdata = dta_c_panel, type='response') # <2>
...
#| 
  ...
  d_plrwt_est = dta_c_panel[censor==0, ] %>% # <2>
    group_by(time) %>% # <2>
...
#| echo: false
  ...
  d_plrstabwt_est = dta_c_panel[censor==0, ] %>% # <2>
    group_by(time) %>% # <2>
    summarize(pr_ev_1 = mean(1-pr_cum_1), 
              pr_ev_0 = mean(1-pr_cum_0),
              .groups = 'drop') %>% # <2>
