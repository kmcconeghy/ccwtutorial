...
data_cloned = bind_rows( # <1>
                     data %>% # <1>
                       mutate(assign = 0, # <1>
                              censor = if_else(treat==1, 1L, NA_integer_), # <1>
                              event = if_else(censor==1, 0, outcome)
                       ), # <1>
                     data %>% # <2>
                       mutate(assign = 1, # <2>
                              censor = if_else(time == 12 & treat !=1, 1L, NA_integer_), # <2>
                              event = if_else(censor==1, 0, outcome)
                              ) # <2>
                       ) %>% # <2>
  arrange(id, assign, time) %>%
  group_by(id, assign) %>%
    mutate(censor = nafill(censor, type = 'locf')) %>%
    mutate(event = nafill(event, type = 'locf')) %>%
  ungroup %>%
  mutate(censor = coalesce(censor, 0L),
         event = coalesce(event, 0L))
```
