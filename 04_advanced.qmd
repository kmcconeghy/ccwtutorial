...
boot_it_1 = function() {
  ...
  d_ids = distinct(dta_c_panel, id) # <1>
  
  d_boot = slice_sample(d_ids, prop=1, replace=T)
  
  d_panel_outc_2 = left_join(d_boot, #<2>
                             dta_c_panel, by = join_by(id),
                             relationship = "many-to-many") # <3>
  
  d_glm_pe = glm(event==0 ~ poly(time, 2, raw=T)*assign, 
                 data=d_panel_outc_2, family=binomial())
  ...
  d_panel_outc_2[, `:=`(pr_cum_1 = cumprod(pr_1)), by=list(id, assign)] # <1>
  d_panel_outc_2[, `:=`(pr_cum_0 = cumprod(pr_0)), by=list(id, assign)] # <1>
...
boot_it_2 = function() {
  ...
  dta_c_panel[, freqwt:=rpois(n=1, lambda=1), by = factor(id)] # <1>
...
#| echo: FALSE
#| tbl-cap: "Comparison of Poisson and Resampling Bootstraps"
